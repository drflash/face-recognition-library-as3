package com.oskarwicha.images.FaceDetection
{
	...
	public class FaceDetector extends EventDispatcher
	{
		...
		private function initLoader():void
		{
			...
			faceImage.contentLoaderInfo.addEventListener(Event.COMPLETE,
		...
		private function initDetector():void
		{
			...
			detector.addEventListener(ObjectDetectorEvent.DETECTION_COMPLETE,
				function(e:ObjectDetectorEvent):void
				{
					logger("[Odebrano zdarzenie typu ObjectDetectorEvent.COMPLETE]");
					detector.removeEventListener(ObjectDetectorEvent.DETECTION_COMPLETE,
						arguments.callee);
					
					if (e.rects)
					{
						e.rects.forEach(function(r:Rectangle,
							idx:int, arr:Array):void
							{
			...
			var events:Array =
				[ObjectDetectorEvent.DETECTION_START,
				ObjectDetectorEvent.HAARCASCADES_LOAD_COMPLETE,
				ObjectDetectorEvent.HAARCASCADES_LOADING];
			events.forEach(function(t:String, idx:int,
				arr:Array):void
				{
					detector.addEventListener(t,
						function(e:ObjectDetectorEvent):void
						{
		...
		private function startDetection():void
		{
			...
			bmpTarget =
				new Bitmap(new BitmapData(faceImage.width,
				faceImage.height, false))
		...
		private function cropImage(loadBitmap:Bitmap,
			startPoint:Point, heightSize:int, widthSize:int):Bitmap
		{
			...
			var cropedBD:BitmapData =
				new BitmapData(widthSize, heightSize, true,
				0x00000000);
			var posPoint:Point = new Point(0, 0);
			// Kopiuje piksele z zaladowanej bitmapy do nowo 
			// utworzonej
			cropedBD.copyPixels(loadBD,
				new Rectangle(startPoint.x, startPoint.y, widthSize,
				heightSize), posPoint);
