package com.oskarwicha.images.FaceRecognition
{
	...
	public class Face extends EventDispatcher
	{
		 ...
		 */
		public function Face(faceUrl:String = null,
			faceClasification:String = null,
			cropFaceFromImg:Boolean = false)
		{
		...
		{
			_crop = crop;
			_imgLoader.contentLoaderInfo.addEventListener(Event.COMPLETE,
				startPreprocessingLoadedImageFromUrl);
			//trace(_faceUrl);
		...
		{
			var imb:Bitmap =
				new Bitmap(new BitmapData(_imgLoader.width,
				_imgLoader.height, false))
			imb.bitmapData.draw(_imgLoader);
			if (_crop)
				detectFaceInPicture(imb, imb);
			var rim:Bitmap =
				this.resizeImage(imb,
				new Rectangle(0, 0,
				FaceRecognition.IDEAL_IMAGE_WIDTH,
				FaceRecognition.IDEAL_IMAGE_HEIGHT), true);
		 ...
		 *
		 */
		public function loadFaceImageFromBitmap(faceBitmap:Bitmap,
		 ...
		 *
		 */
		public function detectFaceInPicture(source:Bitmap,
			destination:Bitmap):void
		{
			var faceDetector:FaceDetector = new FaceDetector();
			
			faceDetector.addEventListener(FaceDetectorEvent.NO_FACES_DETECTED,
				function onNoFacesDetected(e:Event):void
				{
					trace("Uwaga: Twarz nie znaleziona na zdjęciu");
				});
			faceDetector.addEventListener(FaceDetectorEvent.FACE_CROPED,
		 ...
		 */
		private function resizeImage(origB:Bitmap, box:Rectangle,
			fitOutside:Boolean):Bitmap
		{
			var origBD:BitmapData = origB.bitmapData;
			var resizedBD:BitmapData =
				new BitmapData(box.width, box.height, true,
				0x00000000);
			...
			var clipRect:Rectangle =
				new Rectangle(x, y, wNew, hNew);
			
			matrix.scale(ratio, ratio);
			var resizedB:Bitmap = new Bitmap();
			
			resizedBD.draw(origBD, matrix, null, null, clipRect,
				true);
